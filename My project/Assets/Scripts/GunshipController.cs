...
public class GunshipController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            GameObject spawnedCannonball = Instantiate(cannonballPrefab, new Vector3(leftCannon.position.x, leftCannon.position.y), Quaternion.identity);
            spawnedCannonball.GetComponent<Rigidbody2D>().AddForce(cannonballForce * leftDirection);
            
        }
        if (Input.GetMouseButtonDown(1))
        {
            GameObject spawnedCannonball = Instantiate(cannonballPrefab, new Vector3(rightCannon.position.x, leftCannon.position.y), Quaternion.identity);
            spawnedCannonball.GetComponent<Rigidbody2D>().AddForce(cannonballForce * rightDirection);
        }
